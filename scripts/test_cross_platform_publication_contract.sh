...
require 'options: [validate, publish]' "$JVM_CENTRAL_WORKFLOW"
require "inputs.operation == 'publish'" "$JVM_CENTRAL_WORKFLOW"
...
require 'options: [validate, publish]' "$RUST_RELEASE_WORKFLOW"
require 'Validate crates.io token injection without publication' "$RUST_RELEASE_WORKFLOW"
require 'cargo login --registry crates-io' "$RUST_RELEASE_WORKFLOW"
require "inputs.operation == 'publish'" "$RUST_RELEASE_WORKFLOW"
