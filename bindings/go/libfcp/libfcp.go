...
const (
	...
	// EnvelopeIDBytes is the exact signed FCP envelope identifier width.
	EnvelopeIDBytes = 32
	statusOK        = 0
	statusNoAction  = 6
)
...
func (connection *Connection) TakeAction() (*Action, error) {
	...
	envelopeID := C.GoBytes(unsafe.Pointer(&raw.envelope_id[0]), C.int(EnvelopeIDBytes))
	remoteEndpoint, err := copyBuffer(raw.remote_endpoint)
	if err != nil {
		return nil, err
	}
...
func ensureVersions() error {
	if uint32(C.fcp_ffi_abi_version()) != 2 || uint32(C.fcp_ffi_wire_version()) != 1 {
		return fmt.Errorf("libfcp native ABI or wire version is incompatible with this Go façade")
	}
	return nil
}
